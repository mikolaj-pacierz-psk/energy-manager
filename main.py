...
def main():
    ...
    add_device_button = ttk.Button(util_frame, text="Add Device",
                                   command=lambda: show_add_device_window(energy_manager, table, total_energy_label,
                                                                          total_price_label))
    load_file_button = ttk.Button(util_frame, text="Load from CSV",
                                  command=lambda: show_load_file_window(energy_manager, table, total_energy_label,
                                                                        total_price_label))
    save_file_button = ttk.Button(util_frame, text="Save to CSV", command=lambda: show_save_file_window(energy_manager))
    create_energy_chart_button = ttk.Button(chart_frame, text="Create energy chart",
                                            command=lambda: show_energy_chart(energy_manager))
    create_price_chart_button = ttk.Button(chart_frame, text="Create price chart",
                                           command=lambda: show_price_chart(energy_manager))
